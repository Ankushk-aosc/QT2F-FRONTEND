...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const fullUrl = `${backendBaseUrl.replace(/\/$/, "")}/connections/${id}/rename`;
    console.log("[PATCH /api/tableau/connections/[id]/rename] Forwarding to:", fullUrl);
    ...
    if (!backendResponse.ok) {
      ...
      console.error(`[PATCH connections rename] Backend failed: ${backendResponse.status} - ${errorText}`);
      return NextResponse.json(
    ...
    console.error("[PATCH /api/tableau/connections/[id]/rename] Error:", err);
    return NextResponse.json(
